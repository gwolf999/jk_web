        ...
        case 9:               
            if (cal_num[1].length == 0) { //首次输入，没有运算符
                ...
                if (cal_num[2].indexOf('.') == -1 || y != '.') { //这个判断条件防止出现重复输入 .
                  ...
                  console.log("输入第二个参数下，判断完成小数点后，不加。原地赋值:"+cal_num);
...
//检测存储器读取键显示在屏幕上的数字，如果再输入数字，则直接更换已经在屏幕上显示数字；
function check_readmem(){
if(store[1] == 'p'){
   store[1] ='';
   document.getElementById("cal_span").innerHTML ="";
   cal_num[0]='';
}else{
    console.log('store[1]='+store[1]);
}
